...
description: >
  ...
  relationship traversal, aggregate functions, and performance analysis. Builds
  efficient queries that respect governor limits and security requirements.
  TRIGGER when: project is a Salesforce org and user asks to write, optimize,
  or review a SOQL query, analyze query performance, or design a selector
  pattern around governor limits.
  DO NOT TRIGGER when: task is about Apex business logic (use sf-apex), LWC
  UI components (use sf-lwc), test execution (use sf-testing), or a non-SOQL
  query language (SQL, GraphQL, etc).
license: MIT
metadata:
  version: "1.1.0"
