...
description: >
  ...
  scoring. Covers Apex classes, triggers, test classes, batch jobs, and reviews of
  existing Apex for bulkification, security, and SOLID principles.
  TRIGGER when: project contains Salesforce metadata (sfdx-project.json, *.cls,
  *.trigger, force-app/main/default/classes or /triggers), or user asks to write,
  review, or fix Apex code, Salesforce triggers, batch jobs, or Apex tests.
  DO NOT TRIGGER when: task is about Lightning Web Components (use sf-lwc),
  SOQL queries only (use sf-soql), Apex test execution/coverage (use sf-testing),
  or non-Salesforce languages.
license: MIT
metadata:
  version: "1.1.0"
