...
description: >
  ...
  and agentic test-fix loops. Runs Apex tests, analyzes coverage, generates test
  patterns, and automatically fixes failing tests with 120-point scoring.
  TRIGGER when: user asks to run, diagnose, fix, or improve Salesforce tests,
  analyze Apex code coverage, or generate test classes/methods for existing Apex
  code in a Salesforce project.
  DO NOT TRIGGER when: task is about writing new production Apex (use sf-apex),
  LWC Jest tests (use sf-lwc), SOQL query design (use sf-soql), or non-Salesforce
  test frameworks.
license: MIT
metadata:
  version: "1.1.0"
