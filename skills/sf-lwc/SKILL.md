...
description: >
  ...
  TRIGGER when: project contains Lightning Web Components (force-app/main/default/lwc,
  *.js-meta.xml beside *.js/*.html/*.css component bundles), or user asks to
  build, review, or fix an LWC component, wire service, or LWC Jest test.
  DO NOT TRIGGER when: task is about Apex-only code (use sf-apex), SOQL only
  (use sf-soql), Aura components (not covered here), or non-Salesforce frontend
  frameworks (React/Vue/Solid — use the relevant frontend skill).
license: MIT
metadata:
  version: "2.1.0"
